...
pub struct Interpreter<'a> {
    ...
    value_resolution_handlers:
        HashMap<ExprKind, fn(&'a mut Interpreter<'a>, ExprNode) -> RuntimeValue>,
}
...
impl<'a> Interpreter<'a> {
    pub fn new(env: &'a mut Environment) -> Self {
        let value_resolution_handlers = HashMap::from([
            (
                ExprKind::Int,
                Interpreter::resolve_int as fn(&'a mut Interpreter<'a>, ExprNode) -> RuntimeValue,
            ),
    ...
    pub fn resolve_int(&mut self, node: ExprNode) -> RuntimeValue {
        match node {
            ExprNode::Int(value) => RuntimeValue::Int(IntValue {
                id: Counter.next(),
                value,
            }),
    ...
    pub fn resolve_float(&mut self, node: ExprNode) -> RuntimeValue {
        match node {
            ExprNode::Float(value) => RuntimeValue::Float(FloatValue {
                id: Counter.next(),
                value,
            }),
    ...
    pub fn resolve_string(&mut self, node: ExprNode) -> RuntimeValue {
        match node {
            ExprNode::String(value) => RuntimeValue::String(StringValue {
                id: Counter.next(),
                value,
            }),
    ...
    pub fn resolve_bool(&mut self, node: ExprNode) -> RuntimeValue {
        match node {
            ExprNode::Bool(value) => RuntimeValue::Bool(BoolValue {
                id: Counter.next(),
                value,
            }),
