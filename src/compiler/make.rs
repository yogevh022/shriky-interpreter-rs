...
pub(crate) fn make_map(compiler: &mut Compiler, code_object: &mut CodeObject, map: MapNode) {
...
fn make_runtime_map(compiler: &mut Compiler, code_object: &mut CodeObject, object_node: MapNode) {
    let obj_size = object_node.properties.len() * 2;
    object_node.properties.into_iter().for_each(|property| {
        compiler.compile_expr(code_object, property.key, &CompileContext::Normal);
        compiler.compile_expr(code_object, property.value, &CompileContext::Normal);
    });
    compiler.push_op(code_object, OpIndex::with_op(ByteOp::MakeMap, obj_size));
}

pub(crate) fn make_list(compiler: &mut Compiler, code_object: &mut CodeObject, list: ListNode) {
...
fn make_runtime_list(compiler: &mut Compiler, code_object: &mut CodeObject, list_node: ListNode) {
    let list_len = list_node.elements.len();
    list_node.elements.into_iter().for_each(|element| {
        compiler.compile_expr(code_object, element, &CompileContext::Normal);
    });
    compiler.push_op(code_object, OpIndex::with_op(ByteOp::MakeList, list_len));
}
...
pub(crate) fn make_method(
    ...
    let method_const_index = cache_constant(code_object, method_id, method_value);
    compiler.push_op(
        code_object,
...
pub(crate) fn make_class(
    ...
    let class_inheritance_operand = if let Some(boxed_superclass) = class_node.superclass {
        if let ExprNode::Identity(superclass) = *boxed_superclass {
            identity(compiler, code_object, superclass, context);
            1usize
        } else {
